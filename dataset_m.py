...
import numpy as np
def load_train(pos_path,neg_path,ncls):
    allseqs=[]
    labels=[]
    seq_names=[]
    ...
    for eachline in lines:
        ...
        if i%4==0:
            ...
            temp_label=[0]*ncls
            temp_label[int(word[0])-1]=1
            labels.append(temp_label)
            seq_names.append(word[1])
            cls.append(word[0])
            seq=[]
...
def read_train_sets(pos_path, neg_path, pos_path_pred, neg_path_pred, ncls):
  class DataSets(object):
    pass
  data_sets = DataSets()

  train_seqs, train_labels, train_seq_names, train_cls = load_train(pos_path, neg_path, ncls)
  validation_seqs, validation_labels, validation_seq_names, validation_cls = load_train(pos_path_pred, neg_path_pred, ncls)
...
def read_pred_sets(pos_path_pred, neg_path_pred, ncls):
  class DataSets(object):
    pass
  data_sets = DataSets()
